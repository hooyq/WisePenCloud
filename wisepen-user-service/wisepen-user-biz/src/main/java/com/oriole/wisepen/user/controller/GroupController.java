...
public class GroupController {
	...
	@Operation(
			...
			description = """
					...
					失败：身份不允许创建目标小组类型或字段校验失败时按统一异常返回。
					响应：返回新建小组 ID。
					"""
	)
	...
	@Operation(
			...
			description = """
					...
					失败：目标小组不存在、当前用户不是 OWNER、目标类型不允许或字段校验失败时按统一异常返回。
					响应：成功时返回空结果。
					"""
	)
	...
	@Operation(
			...
			description = """
					...
					失败：目标小组不存在、当前用户不是 OWNER、余额转移失败或成员清理失败时按统一异常返回；资源服务通知失败会记录日志但不阻断本接口成功返回。
					响应：成功时返回空结果。
					"""
	)
	...
	@Operation(
			...
			description = """
					...
					失败：目标小组不存在或当前用户不是小组管理员时按统一异常返回。
					响应：返回小组详细信息。
					"""
	)
