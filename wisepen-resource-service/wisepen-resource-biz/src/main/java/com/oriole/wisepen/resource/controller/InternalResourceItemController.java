...
public class InternalResourceItemController implements RemoteResourceService {
    ...
    @Operation(
            ...
            description = """
                    ...
                    失败：个人标签根节点不存在、路径标签非法、资源字段校验失败或搜索同步失败时按统一异常返回。
                    响应：返回新注册资源 ID。
                    """
    )
    @PostMapping("/addRes")
    ...
    @Operation(
            ...
            description = """
                    ...
                    失败：请求参数非法或权限计算依赖数据异常时按统一异常返回；资源不存在按无权限响应处理。
    ...
    @Operation(
            ...
            description = """
                    ...
                    失败：groupId 格式非法或标签、配置清理失败时按统一异常返回。
                    响应：成功时返回空结果。
                    """
    )
